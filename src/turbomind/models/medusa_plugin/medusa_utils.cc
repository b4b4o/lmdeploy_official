...
void MedusaPathTree::getOutputIds(const int* output_preds, int* output_ids, int* each_path_len, const int medusa_head_num){
    // input:
...
void MedusaPathTree::getBatchedOutputIds(const int* output_preds_batched, int* output_ids_batched, int* each_path_len, const int medusa_head_num, const int batch_num){
    // input:
...
void MedusaPathTree::getBatchedPseudoIdsFromTree(const int* medusa_preds_batched, const int medusa_head_num, const int top_k, const int* max_match_count, const int* max_match_idx, int* pseudo_inputs_batched, const int batch_size){
    ...
    for(int b_id = 0; b_id < batch_size; b_id++){
        ...
        std::cout << "[debug] first pseudo_inputs: " << *pseudo_inputs << std::endl; 
    }
