                            ...
                            const MedusaWeight<T>& medusa_weight)
{
    ...
    // medusa_head_logits_buf_: [medusa_head_num, batch_size, vocab_size]
    {
        std::vector<T>tmp(10);
        check_cuda_error(cudaMemcpyAsync(tmp.data(), medusa_head_logits_buf_, sizeof(T) * 10, cudaMemcpyDefault, stream_));
        std::cout << "after medusa linear :" << std::endl;
        for(int i = 0;i < 10;i++)std::cout << " " << tmp[i];
        std::cout << std::endl;
    }
...

    {
        ...
        check_cuda_error(cudaMemcpyAsync(tmp.data(), topk_output_ids, sizeof(int) * 10, cudaMemcpyDefault, stream_));
...
void MedusaHead<T>::allocate_buffer(size_t batch_size)
{
    ...
    // topk_output_ids_t_ =
        // (int*)allocator_->reMalloc(topk_output_ids_t_, sizeof(int) * batch_size * medusa_num_heads_ * max_k_, false);
...
void MedusaHead<T>::top_k(int* d_topk_output_ids, const T* d_input_logits, const size_t batch_size, const int k)
{
    ...
    // cudaMemcpyAsync(d_topk_output_ids, topk_output_ids_t_, sizeof(int) * output_size, cudaMemcpyDeviceToHost, stream_);
